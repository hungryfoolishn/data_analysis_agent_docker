...
## Batch 16: Execution-Before-Compute Plan

- AnalysisPlan is the persisted contract for goal, success criteria, assets,
  fields, ordered steps, dependencies, and resource budget.
- Deterministic validation rejects unknown tools, assets, fields, forward
  dependencies, and plans exceeding the step budget before execution.
- Strict plans must execute only declared methods after declared dependencies
  succeed; legacy dynamic runtime steps remain compatible during migration.

## Batch 17: Second Formal Analysis Methods

- Formal methods now cover distribution, association, period comparison, and
  explicit ratio calculations with Pydantic output and CSV artifacts.
- Every method records sample size, exclusions, denominator or pairwise-complete
  rules, and non-causal/descriptive disclosures where applicable.
- Funnel, retention, group-difference, concentration, and sensitivity methods are
  included with independent boundary tests; they remain descriptive and non-causal.

## Remaining P1: Quality, Task Control, and Workbench

- Result-level quality checks are deterministic and return typed errors,
  warnings, and disclosures.
- Task API background execution exposes idempotent creation, status, event
  cursors, replay, and cancellation without changing the legacy chat contract.
- Workbench state exposes task cursor and quality status; durable multi-worker
  task persistence remains P2.

## P2: Persistence, Governance, and Candidate Acceptance

- SQLite WAL provides transactional snapshots, monotonic events, migration, and
  optimistic write conflict detection while JSON remains an export format.
- Optional Bearer authentication, response hardening, redaction, and sensitive
  field classification establish the default single-node governance boundary.
- Candidate acceptance combines evaluation, quality, regression, recovery,
  process, state, and package-hash evidence into one deterministic decision.
